...
def _train(data_dirname, new_agent, past_agents, config):
    ...
    # collect all traces in the destination folder
...
def train(config):
    config.domain = 'water'
    config.tasks = ['FillCups']
    config.data = '/home/lme/HIL-MT/data'
    # config.domain = 'dishes'
    # config.tasks = ['ClearTable']
    # config.data = '/home/lme/HIL-MT/data'
    config.runs = 1
    config.independent = True   # only train in independent mode
    config.full_batch = True
    # all_agents contain agents from task in tasks configuration
    ...
    if config.independent:
        ...
        # do cartisian product on input, result is list of combination of three modes
...
if __name__ == '__main__':
    ...
    args.runs = 1
    for _ in range(int(args.runs) or 1):    # repeat training for number of runs
        train(args)
