...
# For each skills, you need to select a model to train. models in the model.py
# following 3 classes create relationships and attributes in sql for subskills, agentskill, skillmodel

class SubSkill(db.Model):
    __tablename__ = 'sub_skills'
    # initialize columns domain_name, etc in sql for subskill
    # where is the foreignkey reference to domain_name of agent skill, where agentskill include all skills?
...
class AgentSkill(db.Model):
    ...
    # columns for agent_skill, domain name is 'DishesEnv', agent_name is 'DishesAgent'?
    ...
    # form a relationship for sub_skill
...
def register(domain_name, agent_name):
    ...
    res = DictTree()
    for skill_name, skill in skillset.items():  # iterating over each skill
        if DEBUG:
            print("Registering {}".format(skill_name))
        skill_model = None
        if len(skill.sub_skill_names) > 0:  # If Skill has subskill,initialize skill_model object instead of using None.
        ...
        # Create AgentSkill object for each skill
    ...
    # double for loop, create SubSkill for every subskill exist in skillset
...
def _train(agent_skill, shared_data, validate=True, model_dirname=None):
    ...
    if validate:
        ...
        kf = ms.KFold(num_folds, True)      # k-fold validation. Try num_folds num of training set and testing set.
        validation = []
        roundcnt = 0
        for new_train_idxs, valid_idxs in kf.split(agent_skill.data):
            roundcnt+=1
            print('roundcont', roundcnt)
            print('size:',len(agent_skill.data))
            ...
            # train_data = _process(agent_skill, [agent_skill.data[idx] for idx in range(0,len(agent_skill.data))] + shared_data)    #modification: valid_idxs
            # valid_data = _process(agent_skill, [agent_skill.data[idx] for idx in range(0,len(agent_skill.data))] + shared_data)    #modification: valid_idxs
