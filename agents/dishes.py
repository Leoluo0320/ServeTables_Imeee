...
class MoveObject(hierarchical.Skill):
    model_name = 'table|log_poly2'  # know it is a table. just take a test.
    arg_in_len = 2
    max_cnt = 3
...
class MoveToObject(hierarchical.Skill): # has subskill locate object
    # these parameter did not used in programming, just explanation
    model_name = 't_log_poly2'  # log_poly3 work but need more data
    arg_in_len = 3
    max_cnt = 4
    ...
    def step(arg, cnt, ret_name, ret_val, obs):
        ...
        if cnt == 0:
            ...
            return 'LocateObject', [motion_cnt, obj_class, obj_color] #pixel location
        elif cnt == 2:
            [found_obj_class, found_obj_color, obj_pixel_x, obj_pixel_y] = ret_val
            if DishesEnv.obj_classes[found_obj_class] is None:
                return None, [found_obj_class, found_obj_color] # terminate
            else:
                return 'MoveToLocation', [found_obj_class, found_obj_color, obj_pixel_x, obj_pixel_y]   #found, move to location
        else:
            return None, ret_val
    ...
    def step(arg, cnt, ret_name, ret_val, obs):
        ...
        if cnt == 0:
            if PRETRAINED:
                ...
                item_name = DishesEnv.obj_colors[int(round(obj_color))]+"_"+DishesEnv.obj_classes[int(round(obj_class))]
                if int(round(obj_class)) == 2:
                    armlength = 0.55
                else:
                    armlength = 0.54
                y_adjust = -0.05
                theta_0 = 1.55
                if obj_class == 2 and obj_color == 2:
                    print('error time')
                ...
                print(pos_dict["hsrb"])
                # only works when hsrb face the direction of +y axi. In this case, x for hsrb is +y in gazebo, y for hsrb is -x in gazebo
                y = -(pos_dict[item_name].x - pos_dict["hsrb"].x)+y_adjust
                x = pos_dict[item_name].y - armlength - pos_dict["hsrb"].y
                print('Move to Location execute: {} and {}'.format(x, y))
                return 'MoveBaseRel', [x, y, theta, V, OMEGA]   # take pixel location and move to it. constant: v: velocity, omega:angular v
            else:
...
class DishesAgent(hierarchical.HierarchicalAgent):
    ...
    default_model_name = 'log_poly2'    # default for all skills in dishes. can change by specify in line 162
