...
class HierarchicalAgent(agent.Agent):
    def __init__(self, config):
        ...
        # get skills in (ex. actions in hsr.py and skills in agents/dishes.py)
        ...
        if config.rollable and not config.teacher:  # enter when demonstrating and not teaching
            for skill_name, skill in self.skillset.items():
                if skill.sub_skill_names:
                    # continue
                    # if skill_name in ['CheckWaterAmount', 'MoveToLocation']:
                    #     continue
                    # else:
                    skill.step = load_skill(config.model_dirname, skill_name,
                                            skill)  # do not use skill train, but used the one in sim
                    # modification: change skill.step of MoveToLocation from trained model to my modified function.
                # if skill_name == 'FillCup':
                #     skill.step = ag_water.FillCup.step
                # if skill_name == 'MoveToLocation':
                #    skill.step = ag_water.MoveToLocation.step
                # if skill_name == 'GoToCup':
                #     skill.step = ag_water.GoToCup.step
                # if skill_name == 'PourWater':
                #     skill.step = ag_water.PourWater.step
    ...
    def step(self, obs):
        ...
        while self.stack:
            ...
            print(
                "{}({}, {}, {}, {}) -> {}({})".format(top.name, top.arg, top.cnt, ret_name, ret_val, sub_name, sub_arg))
            if sub_name is None:
                self.stack.pop()
                ret_name = top.name
                ret_val = sub_arg
...
def load_skill(model_dirname, skill_name, skill):
    ...
    def step(arg, cnt, ret_name, ret_val, obs):
        ...
        obs = []  # modification: add line and change obs to list
