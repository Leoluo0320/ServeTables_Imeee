...
def catalog(config):
    ...
    Args:
        config (DictTree)
    """
    if '|' in config.name:  # name give what model to use ex: model_name = 'table|log_poly2'
        return ModelSelector(config)
    elif config.name.startswith('t_'):  # if dependent on cnt in skill, this is time-dependent model need models for each cnt
    ...
    elif config.name.startswith('log_lin'):  # name is log_lin, then use logisticLinear
        return LogisticLinearModel(DictTree(
            num_sub=config.num_sub,
        ))
    elif config.name.startswith('log_poly'):  # name is log_poly, then use logistic with quadratic feature ex: log_poly2
        return LogisticPolynomialModel(DictTree(
            num_sub=config.num_sub,
            degree=int(config.name[len('log_poly'):]),
        ))
    elif config.name.startswith('log_mlp'):  # take more data
    ...
    elif config.name == 'table':    # just a look up table
...
def validate(model, data, sub_arg_accuracy=None):
    ...
    sub_corr = sub_corr.all()   # if all prediction is correct
    arg_mse = ((pred.arg - data.oput.arg) ** 2).sum(0)
    if DEBUG:
        arg_rmse = (arg_mse / len(data.iput)) ** .5
        arg_corr = (arg_rmse <= sub_arg_accuracy).all()
        if not arg_corr and data.oput.sub.size > DEBUG_OUTPUT_THRESHOLD:
            np.set_printoptions(threshold=np.inf)
            print('data input: ', data.iput)
            print('data output: ', data.oput.sub)
            print('predict subskill: ', pred.sub)
            print('data subargument: ', data.oput.arg)
            print('predict subargument: ', pred.arg)
            print('sub_corr: ', sub_corr)
            print('arg_rmse: ', arg_rmse)

    if sub_arg_accuracy is None:
        return DictTree(data_len=len(data.iput), sub_corr=sub_corr, arg_mse=arg_mse)
    else:  # fit the accuarcy, if not correct return arg_mse
...
# ex learn a linear function, train with fit and test with predict.
...
class TimeDependentModel(object):
    ...
    def predict(self, iput):
        ...
        for i in iput:  # take input, find cnt number, get the model for the cnt and predict.
...
class TwoStepModel(object):
    ...
    def fit(self, data):
        ...
        for sub in range(self.num_sub):
            ...
            # print('data iputlll: ', data.iput)
            if DEBUG and len(data.iput[step_idxs[sub]]) > 1 and len(data.iput) > DEBUG_OUTPUT_THRESHOLD*19:
                print('subskill No.', sub)
                np.set_printoptions(threshold=np.inf)
                if isinstance(self.arg_models[sub], lm.LinearRegression):
                    # if self.config.name.startswith('log_lin'):
                    print('model coefficient: ', self.arg_models[sub].coef_)
                    print('model intercept: ', self.arg_models[sub].intercept_)
                    print('model featured input: ', data.iput[step_idxs[sub]])
                elif isinstance(self.arg_models[sub], pl.Pipeline):
                    print('model coefficient: ', self.arg_models[sub].named_steps['linear'].coef_)
                    print('model intercept: ', self.arg_models[sub].named_steps['linear'].intercept_)
                    print('model featured input: ', self.arg_models[sub].named_steps['poly'].transform(data.iput[step_idxs[sub]]))


    def predict(self, iput, only_sub=False):    # iput could be multiple input
        if self.const_sub is None:      # predict sub-skill
        ...
        if only_sub:
            ...
            for inp, sub in zip(iput, pred_sub):
                arg = self.arg_models[sub].predict([inp])   # different arg_model for diff sub-skill
                pred_arg.extend(arg)
            return DictTree(sub=pred_sub, arg=pred_arg)


class LogisticLinearModel(TwoStepModel):    # 2 step model, 1 step: choose sub-skill, base on sub-skill, give argument
    def _make_sub_model(self, config):
...
class TableModel(object):
    ...
    def predict(self, iput):
        pred = [self.table.get(self.disc(i), self.const) for i in iput]  # if not found, output const.
